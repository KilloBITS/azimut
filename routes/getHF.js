...
router.get('/', function(req, res, next){
  ...
  mongoClient.connect(global.baseIP,{ useNewUrlParser: true }, function(err, client){
    ...
    news.find({type:'HF'}).sort({AI: -1}).toArray(function(err, resNews){
      conf.find().toArray(function(err, resultDB){
        var current_page = page;
        var paginator = new pagination.SearchPaginator({prelink: '/HF?' , current: current_page, rowsPerPage: 12, totalResult: resNews.length-1});
